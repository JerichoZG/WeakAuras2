...
function Private.PrintHelp()
	...
	print(
		L[
			'/wa pstart - Start profiling. Optionally include a duration in seconds after which profiling automatically stops. To profile the next combat/encounter, pass a "combat" or "encounter" argument.'
		]
	)
	...
	print(
		L[
			"If you require additional assistance, please open a ticket on GitHub or visit our Discord at https://discord.gg/wa2!"
		]
	)
end
...
function WeakAuras.RegisterSubRegionType(
	...
	elseif (addDefaultsForNewAura and type(addDefaultsForNewAura) ~= "function") then
		error(
			"Improper arguments to WeakAuras.RegisterSubRegionType - addDefaultsForNewAura function is not nil or a function",
			2
		)
		...
		if (enable) then
			...
			if not (arg.name or arg.hidden) then
				...
				if (arg.optional and skipOptional) then
					-- Do nothing
				elseif
					(arg.hidden or arg.type == "tristate" or arg.type == "toggle" or arg.type == "tristatestring" or
						(arg.type == "multiselect" and trigger["use_" .. name] ~= nil) or
						((trigger["use_" .. name] or arg.required) and trigger[name]))
				 then
					...
					elseif (arg.type == "multiselect") then
						...
						elseif (trigger["use_" .. name]) then -- single selection
							...
							if not arg.test then
								test =
									trigger[name] and trigger[name].single and
									"(" .. name .. "==" .. (tonumber(value) or "[[" .. value .. "]]") .. ")"
							else
								test =
									trigger[name] and trigger[name].single and
									"(" .. arg.test:format(tonumber(value) or "[[" .. value .. "]]") .. ")"
							end
						end
					...
					elseif (arg.type == "longstring" and trigger[name .. "_operator"]) then
						...
						test =
							"(" .. name .. (trigger[name .. "_operator"] or "==") .. (number or "[[" .. (trigger[name] or "") .. "]]") .. ")"
					end
...
local function LoadCustomActionFunctions(data)
	...
	if (data.actions) then
		if (data.actions.init and data.actions.init.do_custom and data.actions.init.custom) then
			local func =
				WeakAuras.LoadFunction("return function() " .. (data.actions.init.custom) .. "\n end", id, "initialization")
		...
		if (data.actions.start) then
			if (data.actions.start.do_custom and data.actions.start.custom) then
				local func =
					WeakAuras.LoadFunction("return function() " .. (data.actions.start.custom) .. "\n end", id, "show action")
		...
		if (data.actions.finish) then
			if (data.actions.finish.do_custom and data.actions.finish.custom) then
				local func =
					WeakAuras.LoadFunction("return function() " .. (data.actions.finish.custom) .. "\n end", id, "hide action")
	...
	for id, aura in pairs(WeakAurasSaved.displays) do
		if not aura.ignoreWagoUpdate and aura.url and aura.url ~= "" then
			...
			if slug and version then
				...
				if
					wago and wago.wagoVersion and
						tonumber(wago.wagoVersion) > (aura.skipWagoUpdate and tonumber(aura.skipWagoUpdate) or tonumber(version))
				 then
...
Broker_WeakAuras =
	...
	"WeakAuras",
	{
		...
		OnEnter = function(self)
			...
			local elapsed = 0
			local delay = 1
			tooltip_update_frame:SetScript(
				"OnUpdate",
				function(self, elap)
	...
	local loginThread =
		...
		function()
			...
			for id, data in pairs(db.displays) do
				if (id ~= data.id) then
					print(
						"|cFF8800FFWeakAuras|r detected a corrupt entry in WeakAuras saved displays - '" ..
							tostring(id) .. "' vs '" .. tostring(data.id) .. "'"
					)
					data.id = id
				end
				tinsert(toAdd, data)
...
local function LoadEncounterInitScriptsImpl(id)
	...
	if (id) then
		...
		if
			(data and data.load.use_encounterid and not Private.IsEnvironmentInitialized(id) and data.actions.init and
				data.actions.init.do_custom)
		 then
		...
		for id, data in pairs(db.displays) do
			if
				(data.load.use_encounterid and not Private.IsEnvironmentInitialized(id) and data.actions.init and
					data.actions.init.do_custom)
			 then
				Private.ActivateAuraEnvironment(id)
				Private.ActivateAuraEnvironment(nil)
...
local function scanForLoadsImpl(toCheck, event, arg1, ...)
	...
	local player, realm, spec, zone =
		UnitName("player"),
		GetRealmName(),
		WeakAuras.IsClassic() and 1 or GetSpecialization(),
		GetRealZoneText()
...
function Private.RegisterLoadEvents()
	...
	unitLoadFrame:SetScript(
		"OnEvent",
		function(frame, e, arg1, ...)
			Private.StartProfileSystem("load")
			if (arg1 == "player") then
	...
	if (num > 0) then
		...
		StaticPopupDialogs["WEAKAURAS_RESOLVE_COLLISIONS"] = {
			...
			OnAccept = function(self)
				...
				if
					(WeakAuras.OptionsFrame and WeakAuras.OptionsFrame() and WeakAuras.displayButtons and
						WeakAuras.displayButtons[currentId])
				 then
		...
		if type(data.uid) == "string" then
			if seenUIDs[data.uid] then
				if not silent then
					prettyPrint(
						'duplicate uid "' ..
							data.uid .. '" detected in saved variables between "' .. data.id .. '" and "' .. seenUIDs[data.uid].id .. '".'
					)
				end
...
function Private.SyncParentChildRelationships(silent)
	...
	for id, data in pairs(db.displays) do
		if data.parent then
			...
			elseif not db.displays[data.parent].controlledChildren then
				if not silent then
					prettyPrint(
						"detected corruption in saved variables: " ..
							id .. " thinks " .. data.parent .. " controls it, but " .. data.parent .. " is not a group."
					)
				end
				data.parent = nil
			else
	...
	for id, data in pairs(parents) do
		...
		for index, childID in ipairs(data.controlledChildren) do
			...
			if not child then
				if not silent then
					prettyPrint(
						"detected corruption in saved variables: " .. id .. " thinks it controls " .. childID .. " which doesn't exist."
					)
				end
			...
			elseif child.parent ~= id then
				if not silent then
					prettyPrint(
						"detected corruption in saved variables: " .. id .. " thinks it controls " .. childID .. " which it does not."
					)
				end
				childrenToRemove[index] = true
			elseif groupChildren[childID] then
				if not silent then
					prettyPrint(
						"detected corruption in saved variables: " .. id .. " has " .. childID .. " as a child in multiple positions."
					)
				end
				childrenToRemove[index] = true
			else
	...
	for id, data in pairs(children) do
		if not childHasParent[id] then
			if not silent then
				prettyPrint(
					"detected corruption in saved variables: " .. id .. " should be controlled by " .. data.parent .. " but isn't."
				)
			end
...
local function validateUserConfig(data, options, config)
	...
	for index, option in ipairs(options) do
		if not customOptionIsValid(option) then
			prettyPrint(
				data.id ..
					" Custom Option #" .. index .. " in " .. data.id .. " has been detected as corrupt, and has been deleted."
			)
...
function WeakAuras.PreAdd(data)
	...
	local default =
		data.regionType and WeakAuras.regionTypes[data.regionType] and WeakAuras.regionTypes[data.regionType].default
	if default then
		WeakAuras.validate(data, default)
	end

	local regionValidate =
		data.regionType and WeakAuras.regionTypes[data.regionType] and WeakAuras.regionTypes[data.regionType].validate
	...
	if simpleChange then
		...
		if (data.controlledChildren) then
			...
			if data.triggers.disjunctive == "custom" then
				triggerLogicFunc =
					WeakAuras.LoadFunction("return " .. (data.triggers.customTriggerLogic or ""), id, "trigger combination")
			end
...
function Private.HandleChatAction(
	message_type,
	message,
	message_dest,
	message_channel,
	...
	elseif (message_type == "WHISPER") then
		if (message_dest) then
			if
				(message_dest == "target" or message_dest == "'target'" or message_dest == '"target"' or message_dest == "%t" or
					message_dest == "'%t'" or
					message_dest == '"%t"')
			 then
				pcall(
	...
	local function frame_monitor_callback(event, frame, unit)
		...
		if type(glow_frame_monitor) == "table" then
			for region, data in pairs(glow_frame_monitor) do
				if region.state and region.state.unit == unit and (data.frame ~= frame) == update_frame then
		...
		for regionData, data_frame in pairs(Private.dyngroup_unitframe_monitor) do
			if
				regionData.region and regionData.region.state and regionData.region.state.unit == unit and
					(data_frame ~= frame) == update_frame
			 then
				if not new_frame then
					new_frame = WeakAuras.GetUnitFrame(unit) or WeakAuras.HiddenFrames
				end
...
function Private.HandleGlowAction(actions, region)
	if
		actions.glow_action and
			(((actions.glow_frame_type == "UNITFRAME" or actions.glow_frame_type == "NAMEPLATE") and region.state.unit) or
				(actions.glow_frame_type == "FRAMESELECTOR" and actions.glow_frame))
	 then
		local glow_frame
		...
		if glow_frame then
			...
			elseif actions.glow_action == "hide" and region.active_glows_hidefunc and region.active_glows_hidefunc[glow_frame] then
				region.active_glows_hidefunc[glow_frame]()
...
function WeakAuras.GetAuraTooltipInfo(unit, index, filter)
	...
	local tooltipText =
		tooltipTextLine and tooltipTextLine:GetObjectType() == "FontString" and tooltipTextLine:GetText() or ""
		...
		for id, triggers in pairs(triggerState) do
			...
			for triggernum, triggerData in ipairs(triggers) do
				for id, state in pairs(triggerData) do
					if
						state.progressType == "timed" and state.expirationTime and state.expirationTime < t and state.duration and
							state.duration > 0
					 then
...
function Private.RunCustomTextFunc(region, customFunc)
	...
	local custom = {
		select(
			2,
			xpcall(customFunc, geterrorhandler(), expirationTime or math.huge, duration or 0, progress, dur, name, icon, stacks)
		)
	}
	Private.ActivateAuraEnvironment(nil)
	return custom
...
local function ValueForSymbol(symbol, region, customFunc, regionState, regionStates, useHiddenStates, formatters)
	...
	elseif regionState[symbol] then
		...
		local value =
			(useHiddenStates or regionState.show) and
			ReplaceValuePlaceHolders(symbol, region, customFunc, regionState, formatters[symbol])
		return value or ""
	end
end
...
function Private.ReplacePlaceHolders(textStr, region, customFunc, useHiddenStates, formatters)
	...
	if (endPos == 2) then
		if string.byte(textStr, 1) == 37 then
			...
			local value =
				(regionState.show or useHiddenStates) and
				ReplaceValuePlaceHolders(symbol, region, customFunc, regionState, formatters[symbol])
			if (value) then
	...
	while currentPos <= endPos do
		...
		elseif state == 2 then -- Percent Rest State
			if (char >= 48 and char <= 57) or (char >= 65 and char <= 90) or (char >= 97 and char <= 122) or char == 46 then
				...
				local symbol = string.sub(textStr, start, currentPos - 1)
				result =
					result .. ValueForSymbol(symbol, region, customFunc, regionState, regionStates, useHiddenStates, formatters)
	...
	personalRessourceDisplayFrame.OptionsClosed = function()
		...
		if (frame) then
			...
			elseif (ElvUIPlayerNamePlateAnchor) then
				personalRessourceDisplayFrame:Attach(
...
function Private.AnchorFrame(data, region, parent)
	if
		data.anchorFrameType == "CUSTOM" and (data.regionType == "group" or data.regionType == "dynamicgroup") and
			not WeakAuras.IsLoginFinished() and
			not anchorFrameDeferred[data.id]
	 then
		...
		if
			(data.anchorFrameParent or data.anchorFrameParent == nil or data.anchorFrameType == "SCREEN" or
				data.anchorFrameType == "MOUSE")
		 then
